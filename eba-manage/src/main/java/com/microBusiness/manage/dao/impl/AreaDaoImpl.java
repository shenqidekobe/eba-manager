...
public class AreaDaoImpl extends BaseDaoImpl<Area, Long> implements AreaDao {
	...
	public Area findByFullName(String fullName) {
		try {
			String jpql = "select area from Area area where area.fullName=:fullName order by area.grade asc";
			TypedQuery<Area> query = entityManager.createQuery(jpql, Area.class).setParameter("fullName", fullName);
