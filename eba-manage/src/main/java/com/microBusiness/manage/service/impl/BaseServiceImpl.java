...
public abstract class BaseServiceImpl<T extends BaseEntity<ID>, ID extends Serializable> implements BaseService<T, ID> {
	...
	@Transactional
	public void deleted(ID... ids) {
		if (ids != null) {
			for (ID id : ids) {
				deleted(baseDao.find(id));
			}
		}
	}
	...
	@Override
	public void copyEntity(T source, T target, String... ignoreProperties) {
		this.copyProperties(source , target , ignoreProperties);
	}
}