	...
	<body >
		<div class="" style="width:100%;">
			<form id="listForm" action="viewGoods.jhtml" method="get">
				<input type="hidden" name="supplyNeedId" value="${supplyNeed.id}">
				<div class="hang_list">
					<div class="ch_search">
				...
				<div class="table_box">
					...
					<div class="list_t_tbody" id="listTable">
						<table class="table table-border table-hover table_width">
							...
							<tbody>
								[#list page.content as needProduct]
								<tr class="text-l">
									<td>
										${needProduct.products.sn}
										<!-- <span[#if goods.isOutOfStock] class="red"[#elseif goods.isStockAlert] class="blue"[/#if]>
											${goods.sn}
										</span> -->
									</td>
									<td>
										<span title="${needProduct.products.goods.name}">
											${abbreviate(needProduct.products.goods.name, 80, "...")}
										</span>
										[#list needProduct.products.goods.validPromotions as promotion]
											<span class="promotion" title="${promotion.title}">${promotion.name}</span>
										[/#list]
										[#if needProduct.products.goods.source??]
										<span class="distribut_label">分销</span>
										[/#if]
									</td>
									<td>${needProduct.products.goods.productCategory.name}</td>
									<td>
										[#list needProduct.products.specificationValues as specificationValue]
												${specificationValue.value}&nbsp;
										[/#list]
									</td>
									
									<td>
										${currency(needProduct.products.price, true)}<br />
									</td>
									<td>
										${needProduct.supplyPrice}
									</td>

                                    <td>
                                    	${needProduct.minOrderQuantity}
                                    </td>
								</tr>
								[/#list]
							</tbody>
