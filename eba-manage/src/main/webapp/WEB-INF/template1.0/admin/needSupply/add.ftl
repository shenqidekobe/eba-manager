			...
			$("#submitButton").click(function(){
				...
				$.ajax({
		            ...
		            success: function (data) {
		                if(!data.isTrue){
		                	isTrue=false;
		                	$.message({'type':'error' , 'content':'该门店供应关系已经存在，不能重复添加！'});
		                }
		            }
				});
				if(!isTrue){
						...
						<div class="pag_div">
							<div class="row cl" style="float:left;">
								<label class="form-label col-xs-4 col-sm-3">
									<img src="${base}/resources/admin1.0/images/bitian_icon.svg" alt="" />选择门店</label>
								<div class="formControls col-xs-8 col-sm-7" style="position:relative;"> 
									...
									<input type="text" class="downList_val" name="need" id="need"/>
									<ul class="downList_con">
                                        <li val="">${message("admin.common.choose")}</li>
										[#list needList as need]
											<li val="${need.id}">${abbreviate(need.name, 30, "...")}</li>
										[/#list]
									</ul>
								</div>
							</div>
