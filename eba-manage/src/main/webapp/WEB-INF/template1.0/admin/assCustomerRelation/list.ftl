		...
		<div class="child_page"><!--内容外面的大框-->	
			<div class="cus_nav">
				<ul>
					...
					<li>${message("admin.assCustomerRelation.list")} <span>(${message("admin.page.total", page.total)})</span></li>
				</ul>
			</div>
			...
			<div class="ch_condition">
				...
				<div class="ch_search">
					...
					<div class="search_input">
						<input type="text" id="searchName" name="searchName" value="${searchName}" maxlength="200" placeholder="输入供应商名称/供应商编号/联系人" />
				...
				<div class="ch_operate">
					[@shiro.hasPermission name = "admin:assCustomerRelation:add"]
						<button type="button" class="op_button add_B" onclick="add();">${message("admin.common.add")}</button>
					[/@shiro.hasPermission]
					[@shiro.hasPermission name = "admin:assCustomerRelation:delete"]
				...
				<table class="table table-border table-hover table_width">
					<thead>
						<tr class="text-l">
							...
							<th width="15%">${message("CustomerRelation.clientName")}</th>
							<th width="8%">${message("CustomerRelation.clientNum")}</th>
							<th width="15%">${message("CustomerRelation.area")}</th>
							<th width="10%">${message("CustomerRelation.userName")}</th>
							<th width="10%">${message("CustomerRelation.tel")}</th>
					...
					<table class="table table-border table-hover table_width">
						<thead>
							<tr class="text-l">
								...
								<th width="15%"><div class="th_div">${message("CustomerRelation.clientName")}</div></th>
								<th width="8%"><div class="th_div">${message("CustomerRelation.clientNum")}</div></th>
								<th width="15%"><div class="th_div">${message("CustomerRelation.area")}</div></th>
								<th width="10%"><div class="th_div">${message("CustomerRelation.userName")}</div></th>
								<th width="10%"><div class="th_div">${message("CustomerRelation.tel")}</div></th>
								<th width="15%"><div class="th_div">${message("CustomerRelation.createDate")}</div></th>
						...
						[#list page.content as customerRelation]
							<tr class="text-l">
								<td><input type="checkbox" name="ids" value="${customerRelation.id}"></td>
								...
								<td class="td-manage">
									[@shiro.hasPermission name = "admin:customerRelation:edit"]
										<a title="${message("admin.common.edit")}" href="edit.jhtml?id=${customerRelation.id}&oldClientName=${customerRelation.clientName}" class="ml-5" style="text-decoration:none"><i class="operation_icon icon_bianji"></i></a>
									[/@shiro.hasPermission]
								</td>
							</tr>
						[/#list]
