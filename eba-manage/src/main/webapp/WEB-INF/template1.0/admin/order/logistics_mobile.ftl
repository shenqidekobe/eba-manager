        ...
        <div class="info-send">
            <div class="take-info">
                ...
                [@shiro.hasPermission name = "admin:order:cancelShipped"]
                    <button type="button" class="ta-st [@shiro.lacksPermission name = 'admin:order:cancelShipped'] hhhh [/@shiro.lacksPermission]"  onclick="cancelShipping(${order.id} , ${shipping.id})"[#if shipping.status == "senderChecked"] style="display: none" disabled="disabled"[/#if]>作废</button>
                [/@shiro.hasPermission]
