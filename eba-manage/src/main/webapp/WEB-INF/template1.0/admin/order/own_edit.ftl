...
<div class="child_page"><!--内容外面的大框-->
                                        ...
                                        <div class="formControls col-xs-8 col-sm-10">
                                            ...
                                            [#list order.orderItems as orderItem]
                                                <tr class="text-l" name="dataTr">
                                                    ...
                                                    <td><input type="number" min="${orderItem.minOrderQuantity}" class="input-text radius tdInput1 shippingItemsQuantity" name="orderItems[${orderItem_index}].checkQuantity" style="width:50px" value="${orderItem.quantity}"  id="quantity_${orderItem.product.id}" oninput="checkUpdateQuantity(${orderItem.product.id},this);" onpropertychange="checkUpdateQuantity(${orderItem.product.id},this);" />
            ...
            <ul class="goods_ul">
                ...
                [#list products as product]
                    <li class="canChoose" data-sn="${product.sn }" data-name="${product.goods.name }[#if product.specifications?has_content]
                                                        ...
                                                        [/#if]" data-productid="${product.id }" data-price="${product.supplyPrice }" data-minorderquantity="${product.minOrderQuantity }" onclick="addTrData(this);">${product.sn }  ${abbreviate(product.goods.name, 50, "...")} 
    ...
    <script type="text/javascript">
        ...
        $("tr[name=dataTr]").each(function(){
           ...
           product.quantity=$(this).find("#quantity_"+productId).val();
        ...
        function addTrData(obj){
            if($(obj).hasClass("canChoose")){
                ...
                var minorderquantity=$(obj).data("minorderquantity");
                var htmlStr='<td><button name="recovery" type="button" onclick="recoverys(this);" class="restore_B">复原</button><button name="less" class="numLess" type="button" onclick="deleteTr(this);" style="width: 20px;"></button><button name="plus" class="numPlus" type="button" style="width: 20px;" onclick="addTr(this);"></button>'+sn+'</td><td>'+name+'</td><td>1</td><td><input type="number" min="'+minorderquantity+'" class="input-text radius tdInput1 shippingItemsQuantity" name="orderItems['+dtId+'].checkQuantity" style="width:50px" value="'+minorderquantity+'"  id="quantity" oninput="checkUpdateQuantity('+productId+',this);" onpropertychange="checkUpdateQuantity('+productId+',this);" /><input type="hidden" name="orderItems['+dtId+'].id" value="" /><input type="hidden" id="productId" name="orderItems['+dtId+'].productId" value="'+productId+'" /><input type="hidden" name="orderItems['+dtId+'].supplyPrice" value="'+supplyPrice+'" /><input type="hidden" id="status" name="orderItems['+dtId+'].status" value="add" /></td>';
