					...
					<div class="list_t_tbody" id="listTable">
						<table class="table table-border table-hover table_width">
							...
							<tbody>
								...
								<tr class="text-l">
									<td><input type="checkbox" name="ids" id="${product.id}" value="${product.id}" name="sub_checked" onclick="selectGoods(${product.id})"/></td>
									<td>
										${product.sn}
									</td>
									<td>
                                    ...
                                    <td>
										<p class="price_form_p qiding_p" style="width:56px;">
											<input type="number" id="product_orderQuantity_${product.id}" min="1" class="in_no" productId="${product.id}" goodsId="${product.goods.id}" name="product_orderQuantity_${goods.id}" value="1" oninput="javascript:updateOrderQuantity(this);" onpropertychange="javascript:updateOrderQuantity(this);"/>
										</p>
									</td>
								</tr>
								[/#list]
			...
			function selectGoods(id){
		        ...
		        if($id.attr("checked")){
		            ...
		            $.each(inputs, function (i, input) {
		                ...
		                tempJson[id] = {"products":id , "supplyPrice":$(input).val() , "minOrderQuantity":minOrderQuantity};
		            });
