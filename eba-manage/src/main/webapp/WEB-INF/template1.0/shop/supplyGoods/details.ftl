    ...
    <div class="dh-nav-bar-selected">
        <div class="nav-bar-con">
            <div class="nav-bar">
                ...
                <i>/</i>
                <a class="nav-bar-active" href="/shop/companyGoods/getCompanyGoodsList.jhtml?pubType=pub_supply" target="_self" title="供应信息">供应信息</a>
                <sapn style="display:none" class="oneCategory"> 
                <i>/</i>
                <a onclick=queryCategory("pub_supply","${companyGoods.category.parent.parent.id }","${companyGoods.category.parent.parent.id }"); class="nav-bar-active" href="javascript:void(0);" target="_self" title="${companyGoods.category.parent.category.name }">${companyGoods.category.parent.parent.name }</a>
                </sapn>
                <sapn style="display:none" class="twoCategory"> 
                <i>/</i>
                <a onclick=queryCategory("pub_supply","${companyGoods.category.parent.id }","${companyGoods.category.parent.parent.id }","${companyGoods.category.parent.id }"); class="nav-bar-active" href="javascript:void(0);" target="_self" title="${companyGoods.category.parent.name }">${companyGoods.category.parent.name }</a>
                </sapn>
                <sapn style="display:none" class="threeCategory"> 
                <i>/</i>
                <a onclick=queryCategory("pub_supply","${companyGoods.category.id }","${companyGoods.category.parent.parent.id }","${companyGoods.category.parent.id }","${companyGoods.category.id }"); class="nav-bar-active" href="javascript:void(0);" target="_self" title="${companyGoods.category.name }">${companyGoods.category.name }</a>
                 </sapn>
                <i>/</i>
        ...
        <div class="goods-content">
            <div class="goods-con">
                <div class="goods-img-box">
                    ...
                    <img src="${companyGoods.image}" alt="" />
                    </a>
                </div>
                <div class="dh-goods-d company-sh">
                    <div class="goods-detail">
                        ...
                        <div class="goods-info">
                            <p class="price-tag"><i class="goods-par">参考价格：</i>
                            <span class="goods-price">
                             [#if companyGoods.marketPrice == "-1"]
                                    	面议
                             [#else]
                                     ￥${companyGoods.marketPrice }
                             [/#if]
                            </span>
                    ...
                    <input class="goodsId" type="hidden" value="${companyGoods.id}" />
                    <div class="goods-contact">
                        ...
                        [#assign favorCompanyGoodsFlag=false]
                        [#if favorCompanyGoodsL == null]
                            <div class="goods-collect">
                                <img src="${base}/resources/shop/common/images/shoucang-a.svg" alt="" />
                                <span>收藏产品</span>
                            </div>
                        [#else]
                        [#list favorCompanyGoodsL as favorCompanyGoods]
                            [#if companyGoods.id == favorCompanyGoods.companyGoodsId]
                                [#assign favorCompanyGoodsFlag=true]
                                [#break]
                            [/#if]
                        [/#list]
                        [#if favorCompanyGoodsFlag]
                            <div class="goods-collect">
                                <img src="${base}/resources/shop/common/images/shoucang-b.svg" alt="" />
                                <span>取消收藏</span>
                            </div>
                        [#else]
                            <div class="goods-collect">
            ...
            <div class="goods-list-l">
                ...
                <ul class="list-other">
                  [#list supplyCompanyGoodsList.content as supplyCompanyGoods]
                    <li onclick=queryDetails("pub_supply","${supplyCompanyGoods.id }");>
                        <a class="dh-other dh-ne-i" href="javascript:void(0);" target="_self" title="">
