		...
		<div class="child_page"><!--内容外面的大框-->	
			...
			<div class="form_box" style="overflow: auto;">
				<form id="saveMoreForm" action="saveMore.jhtml" method="post">
                    <input type="hidden" name="logId" value="${needImportLog.id}">
				</form>
				...
				<form id="listForm" action="importList.jhtml" method="get" class="form form-horizontal">
					...
					<div class="importTable">
						<p class="I_result">导入情况：本次共导入<span class="I_color">${needImportLog.total}</span>条门店数据，<span class="I_color">${needImportLog.successNum}</span>条正确，<span class="I_color">${needImportLog.errorNum}</span>条错误，只能够导入数据格式正确的门店</p>
						<div class="" id="">
							<table id="productTable" class="table table-border table-hover table_width boo">
				                ...
				                <tr class="text-c">
				                    ...
				                    <td>直营</td>
				                    <td>${supplierTel}</td>
				                    <td title="${info.name} ">${abbreviate(info.name, 40, "...")}</td>
				                    <td>${info.area.fullName} </td>
				                    <td>${info.address}</td>
				                    <td>${info.clerk}</td>
				                    <td>${info.tel}</td>
                                    <td>
