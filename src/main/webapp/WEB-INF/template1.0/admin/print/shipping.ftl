		...
		<div class="print_box">
			...
			<div class="con_table">
				...
				<table class="table table-border table-hover table_width">
					...
					<tr>
						<td>${sippingItem_index + 1}</td>
						<td>${abbreviate(sippingItem.name, 50, "...")}</td>
						<td>
							[#if sippingItem.specifications?has_content]
		                        <span class="silver">[${sippingItem.specifications?join(", ")}]</span>
							[/#if]
						</td>
						<td>${sippingItem.quantity}</td>
					</tr>
					[/#list]
					
					<tr class="last_tr text-r">
						<td colspan="4">共<i>${shipping.getQuantity()}</i>件商品</td>
					</tr>
				</table>
