...
<html>
	...
	<div class="child_page"><!--内容外面的大框-->
		...
		<div class="form_box reportForm">
			<form class="form form-horizontal" id="listForm" >
				<input type="hidden" name="ts" id="ts" />
				<input type="hidden" name="search" id="search"/>
				<div id="tab-system" class="HuiTab">
					<div class="tabBar cl">
						<a href="../commodityReport/index.jhtml"><span>订货商品</span></a>
					...
					<div class="tabCon">
						...
						<div class="i_box orderForm">
							...
							<div class='i_content'>
								...
								<div class="data_li">
									...
									<h3 class="order_p">采购商品数</h3>
								</div>
								<div class="data_li">
									<div class="order_num"><div id="totalAmount">0</div></div>
									<h3 class="order_p">采购商品金额</h3>
								</div>
							</div>
						</div>
						...
						<div class="i_box orderForm">
							<div class="i_title">采购商品构成</div>
							<div class='i_content'>
								<div class="time_slot">
									<div class="switch_button" style="width:60px">
										<input type="button" class="thisWeek select_color" value="按商品" />
										<i class="select_B"></i>
									</div>
								</div>
						...
						<div class="orderTable">
							<table class="table table-border table-hover table_width">
								<thead>
									<tr class="text-l">
										<th width="20%">商品编号</th>
										<th width="25%">商品名称<i id="goodDate"></i></th>
										<th width="15%">采购单数</th>
										<th width="15%">采购商品数</th>
										<th width="10%">采购供应商数</th>
										<th width="15%">采购金额</th>
									</tr>
								</thead>
								<tbody>
	...
	<script type="text/javascript">
		$(function(){
			...
			function loadData() {
				...
				$.ajax({
					...
					success: function(data) {
								...
								if(datas[i].specification == "[]") {
									$(".orderTable tbody").append('<tr>'+
										'<td><label id="productName">'+datas[i].productSn+'</label></td>'+
											...
											'<td><label id="customersNum">'+datas[i].suppliersNum+'</label></td>'+
			    		...
			    		var pieChart = [];
						var legendDat = [];
						console.log(data.commodityReportDtos);
			...
			$("#downButton").click(function () {
				$("#listForm").attr("action", "purchasedGoodsExported.jhtml");
				$("#listForm").submit();
		    });
