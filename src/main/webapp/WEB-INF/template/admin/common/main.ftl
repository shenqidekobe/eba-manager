...
<title>${message("admin.main.title")} - Powered By DreamForYou</title>
...
<script type="text/javascript">
	...
	function downloadQR(){
		window.location.href="../common/downQRCode.jhtml?supplierId=${supplier.id}";
	}
...
$().ready(function() {
    ...
    $("#getQRCode").click(function() {
		var url = "../common/getQRCode.jhtml?supplierId=${supplier.id}";
        $.dialog({
            title: "商城二维码",
			[@compress single_line = true]
                content: '<img src="../common/getQRCode.jhtml?supplierId=${supplier.id}" alt=""\/><input type="button" onclick="javascript:downloadQR();" class="button" style="display:block; margin:0 auto;margin-bottom: 20px;" value="下载二维码" \/>',
			[/@compress]
            width: 300,
            modal: true,
			ok:null,
	...
	$nav.click(function() {
		...
		$menu.hide();
		$currentMenu.show();
		if($currentMenu.find("a").size()>0){
			$currentMenu.find("a")[0].click();
		}
		return false;
	});
	...
	<table class="main">
			...
			<th>
				<div id="nav" class="nav" style="width: auto">
					<ul>
						...
						[#list ["admin:need"] as permission]
							[@shiro.hasPermission name = permission]
                                <li>
                                    <a href="#need">${message("admin.main.needNav")}</a>
                                </li>
								[#break /]
							[/@shiro.hasPermission]
						[/#list]
						...
						[#list ["admin:formalSupply","admin:temporary","admin:supplyDistribution"] as permission]
							[@shiro.hasPermission name = permission]
                                <li>
                                    <a href="#formalSupply">${message("admin.main.formalSupplyNav")}</a>
                                </li>
								[#break /]
							[/@shiro.hasPermission]
						[/#list]

						[#list ["admin:order", "admin:payment", "admin:refunds", "admin:shipping", "admin:returns", "admin:deliveryCenter", "admin:deliveryTemplate" , "admin:ownOrder"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
						...
						[#list ["admin:statistics", "admin:memberStatistic", "admin:orderStatistic", "admin:memberRanking", "admin:goodsRanking"] as permission]
							...
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:setting", "admin:area", "admin:paymentMethod", "admin:shippingMethod", "admin:deliveryCorp", "admin:paymentPlugin", "admin:storagePlugin", "admin:loginPlugin", "admin:admin", "admin:role", "admin:message", "admin:log","admin:enterpriseInfo"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
                ...
                <div class="link" style="width: auto">
					...
					[#if supplier != null]
						[#if supplier.status == 'notCertified' ]
							<a id="" href="../certification/index.jhtml" target="iframe" style="color:red">企业未认证</a>
						[/#if]
						[#if supplier.status == 'authenticationFailed' ]
						 	<a id="" href="../certification/reviewMiss.jhtml" target="iframe" style="color:red">认证失败</a>
						[/#if]
						[#if supplier.status == 'certification' ]
						 	<a id="" href="../certification/reviewing.jhtml" target="iframe" style="color:blue">认证中</a>
						[/#if]
						[#if supplier.status == 'verified' ]
						 	<a id="" href="javascript:void(0);" target="iframe" style="color:green">企业已认证</a>
						[/#if]
                        <a id="getQRCode" href="javascript:void(0);">[商城二维码]</a>
					[/#if]
		...
		<tr>
			<td id="menu" class="menu">
                ...
                <dl id="need">
                    <dt>${message("admin.main.needGroup")}</dt>
					[@shiro.hasPermission name="admin:need"]
                        <dd>
                ...
                <dl id="supplier">
                    <dt>${message("admin.main.supplierManagement")}</dt>
                ...
                <dl id="formalSupply">
                    <dt>${message("admin.main.formalSupplys")}</dt>
					[@shiro.hasPermission name="admin:formalSupply"]
                        <dd>
					...
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:supplyDistribution"]
                        <dd>
				...
				<dl id="order">
					...
					[@shiro.hasPermission name="admin:ownOrder"]
                        <dd>
				...
				<dl id="system">
					...
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:notice"]
                        <dd>
