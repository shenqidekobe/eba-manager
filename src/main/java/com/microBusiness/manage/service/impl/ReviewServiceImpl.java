...
import com.microBusiness.manage.dao.ReviewDao;
...
public class ReviewServiceImpl extends BaseServiceImpl<Review, Long> implements ReviewService {
	...
	public List<Review> findList(Member member, Goods goods, Review.Type type, Boolean isShow, Integer count, List<Filter> filters, List<com.microBusiness.manage.Order> orders) {
		return reviewDao.findList(member, goods, type, isShow, count, filters, orders);
	}

	@Transactional(readOnly = true)
	@Cacheable(value = "review", condition = "#useCache")
	public List<Review> findList(Long memberId, Long goodsId, Review.Type type, Boolean isShow, Integer count, List<Filter> filters, List<com.microBusiness.manage.Order> orders, boolean useCache) {
	...
	public boolean hasPermission(Member member, Goods goods) {
		...
		if (Setting.ReviewAuthority.purchased.equals(setting.getReviewAuthority())) {
			...
			long orderCount = orderDao.count(null, com.microBusiness.manage.entity.Order.Status.completed, member, goods, null, null, null, null, null, null);
